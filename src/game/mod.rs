...
pub fn gameloop()
{
	...
	for x in 0..5
	{
		colony.push(bunny:: Bunny
			{ sex: rng.gen(),
			  color: rng.gen(),
			  name: rng.choose(&bunny::NAMESLIST).unwrap(), // https://habrahabr.ru/post/274485/
			  age: 0,
			  ghoul: false
			});
	...
	loop
	{
		...
		for x in colony.iter_mut() { x.incrementage(); }
...
fn breed( colony: &mut Vec<bunny::Bunny> )
{
	...
	if colony.iter().find(|&male| male.age >= 2 && male.sex == bunny::Sex::Male ) != None
    {
        if colony.iter().find(|&female| female.age >= 2 && female.sex == bunny::Sex::Female) != None
        {
